...
- **Móvil:** 97 tests jest (`cd mobile && npm test`) + `tsc` limpio (sin contar lo que sume el PR #30, mergeado en paralelo a esta rama).
...
### Próximo
- **Slice elegido: división de cuenta por artículos en Caja móvil — Opción A "calculadora de división" (decidido 2026-08-11).** Asignar artículos del pedido a "personas" en la pantalla de cobro; el sistema calcula cuánto le toca a cada quien (con su parte proporcional de IVA) y cada monto se registra como una línea de pago más de la **misma venta** (la API ya acepta N pagos) — un solo folio/ticket, **sin cambios de backend**. La Opción B (ventas/folios separados por comensal) quedó descartada por ahora: rompe el modelo venta 1:1 con pedido. Falta: spec → plan → TDD → PR.
