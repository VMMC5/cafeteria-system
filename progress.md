...
**Última actualización:** 2026-08-11 (**cuenta por mesa — venta multi-pedido**, rama `feature/cuenta-por-mesa`, pendiente de PR; antes varios pedidos por mesa mergeado a `main` — PR #31, squash `6feed1e`; antes división de cuenta por artículos en Caja móvil, PR #30 `ddbca26`)
...
### Próximo
- **Slice elegido: cuenta por mesa — venta multi-pedido, "Opción A" (diseño aprobado 2026-08-11).** Cobrar todas las rondas de una mesa en un solo folio/ticket: la FK se invierte (`pedidos.id_venta` nullable reemplaza a `ventas.id_pedido`, migración con backfill), `POST /ventas` acepta `ids_pedidos` (misma mesa, sin venta previa; con varias rondas, todas Entregadas — bloqueo elegido por el usuario), y Caja agrupa por mesa con cobro de cuenta completa o ronda suelta (mismo endpoint, lista de uno). La calculadora del PR #30 opera sobre la unión de líneas sin cambios de lógica. Se evaluaron y descartaron: cobro por lote en el móvil (N folios para un pago real) y entidad `Cuenta` formal (YAGNI). Falta: spec → plan → TDD → PR. **Depende de #30 y #31, ambos ya en main.**
