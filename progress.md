...
**Última actualización:** 2026-08-09 (**fixes menores mergeados a `main` — PR #27**, squash `b1f0e80`: logout POST, refresh-on-401 móvil, test de pagos múltiples; antes inventario a 3 decimales, PR #26 `84d8161`; antes CSRF en el panel web, PR #25 `64bc9e8`)
...
### Próximo
- **División de cuenta por artículos: rama `feature/split-bill-calculadora` lista para PR.** Spec y plan en `docs/superpowers/` (2026-08-11); implementación TDD completa (Tareas 1–4) y smoke en dispositivo ejecutado y OK. Nota del spec que resultó clave: no hubo prorrateo de IVA — los precios del detalle son finales y el total del pedido es la suma exacta de sus líneas. La Opción B (ventas/folios separados por comensal) sigue descartada: rompe el modelo venta 1:1 con pedido.
